...
public class ServiceCustomer {
    ...
    public ArrayList<ModelMonAn> MenuFoodOrder(String type, String orderBy) throws SQLException {
        ...
        String sql = "SELECT ID_MonAn,TenMon,DonGia FROM MonAn WHERE Loai=? AND TrangThai='Đang kinh doanh'";
        switch (orderBy) {
            case "Tên A->Z" -> {
                sql = "SELECT ID_MonAn,TenMon,DonGia FROM MonAn WHERE Loai=? AND TrangThai='Đang kinh doanh' ORDER BY TenMon";
            }
            case "Giá tăng dần" -> {
                sql = "SELECT ID_MonAn,TenMon,DonGia FROM MonAn WHERE Loai=? AND TrangThai='Đang kinh doanh' ORDER BY DonGia";
            }
            case "Giá giảm dần" -> {
                sql = "SELECT ID_MonAn,TenMon,DonGia FROM MonAn WHERE Loai=? AND TrangThai='Đang kinh doanh' ORDER BY DonGia DESC";
            }
        }
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, type);
    ...
    public ArrayList<ModelBan> MenuTableState(String floor, String state) throws SQLException {
        ...
        switch (state) {
            ...
            case "Còn trống" ->
                sql = "SELECT ID_Ban,TenBan,Trangthai FROM Ban WHERE Vitri=? AND Trangthai='Trống'";
            case "Đã đặt trước" ->
                sql = "SELECT ID_Ban,TenBan,Trangthai FROM Ban WHERE Vitri=? AND Trangthai='Đã đặt trước'";
            case "Đang dùng bữa" ->
                sql = "SELECT ID_Ban,TenBan,Trangthai FROM Ban WHERE Vitri=? AND Trangthai='Đang hoạt động'";
        }
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, floor);
        ResultSet r = p.executeQuery();
    ...
    public void InsertHoaDon(ModelBan table, ModelKhachHang customer) throws SQLException {
        ...
        String sql = "INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai)"
                + " VALUES (?,?,?,to_date(?, 'dd-mm-yyyy'),0,0,'Chưa thanh toán')";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idHD);
    ...
    public ModelHoaDon FindHoaDon(ModelKhachHang customer) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,to_char(NgayHD,'dd-mm-yyyy') AS Ngay,TienMonAn,Code_Voucher,TienGiam,Tongtien,Trangthai FROM HoaDon "
                + "WHERE ID_KH=? AND Trangthai='Chưa thanh toán'";
        PreparedStatement p = con.prepareStatement(sql);
